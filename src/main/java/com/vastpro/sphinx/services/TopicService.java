...
/**
 * This class is used for perform the operation in the topic like create,
 * delete,...
 * 
...
public class TopicService {
	/**
	 * This method is used to create the record in the topic in the topicMaster
	 * table
	 * 
	 * @param dctx   (DispatchContext is used to get the dispatcher and delegator)
	 * @param params (input data to insert into the table)
...
//			}
			...
			GenericValue topicAlreadyExits = EntityQuery.use(delegator).from("topicMaster")
					.where("topicName", topicName, "partyId", partyId).queryFirst();
			...
			// if the topicName is not empty and not present in the table then insert the
			// topic in the table
			Map<String, Object> record = dispatcher.runSync("createTopic", params);
			// any error occur in the insert the record by using entityAuto then return the
			// error message
	 ...
	 * 
	 * @param dctx   (DispatchContext is used to get the dispatcher and delegator)
	 * @param params (input data to update into the table) get the topicId and
	 *               topicName to update in the topicId
	 * @return success or error message
	 */
	public static Map<String, Object> updateTopic(DispatchContext dctx, Map<String, Object> params) {
		...
		try {
			...
			// validate the topicName is already exits if it is exits then return the error
			// message
			GenericValue topicNameAlreadyExits = EntityQuery.use(delegator).from("topicMaster")
					.where("topicName", topicName).queryOne();
...
//			updateMap.put("topicName", topicName);
	 ...
	 * 
	 * @param dctx   (DispatchContext is used to get the dispatcher and delegator)
	 * @param params (input data to delete into the table) get the topicId to delete
	 *               the record
	 * @return success or error message
	 */
	public static Map<String, Object> deleteTopic(DispatchContext dctx, Map<String, Object> params) {
		...
		try {
			...
			GenericValue topicAssignedExam = EntityQuery.use(delegator).from("ExamTopicMapping")
					.where("topicId", topicId).queryFirst();
...
//			deleteMap.put("topicId", topicId);
			// the deleteTopic is delete the record based on the topicId in the topicMaster
			// Table
	...
	public static Map<String, Object> getAllTopics(DispatchContext dctx, Map<String, Object> params) {
		...
		try {
			...
			List<GenericValue> topicList = EntityQuery.use(delegator).from("topicMaster").where("partyId", partyId)
					.orderBy("-lastUpdatedStamp").cursorScrollInsensitive().offset(offset).limit(pageSize).queryList();
	...
	public static Map<String, Object> getTopicById(DispatchContext context, Map<String, Object> input) {
		...
		try {
			...
			GenericValue topicData = EntityQuery.use(delegator).from("topicMaster").where("topicId", topicId)
					.queryOne();
