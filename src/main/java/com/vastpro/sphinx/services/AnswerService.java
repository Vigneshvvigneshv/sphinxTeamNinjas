	...
	public class AnswerService {
		...
		public static Map<String,Object> answerService(DispatchContext dctx,Map<String,Object> context){
			...
			try {
				...
				//Calculating TotalAnswered
				 long totalAnswered = EntityQuery.use(delegator)
				                    .from("AnswerMaster")
				                    .where("partyId", partyId, "examId", examId)
				                    .queryCount();
				 
				 //for getting numberOf Question in the exam
				 GenericValue exam = EntityQuery.use(delegator)
				                    .from("ExamMaster")
				                    .where("examId", examId)
				                    .queryOne();
				 
				  long totalQuestions = exam != null && exam.getLong("noOfQuestions") != null
				                    ? exam.getLong("noOfQuestions") : 0;
				  
				  long remaining = Math.max(0, totalQuestions - totalAnswered);
				
				  
				 Map<String,Object>progressResult= dispatcher.runSync("updateInprogress",UtilMisc.toMap("examId",examId,"partyId",partyId,"totalAnswered",totalAnswered,"totalRemaining",remaining));
				 
				 if(ServiceUtil.isError(progressResult)) {
					 return ServiceUtil.returnError("Error occured in InprogressUpdate");
				 }
				 
				 
				 result.put("totalAnswered", totalAnswered);
				 result.put("totalRemaining", remaining);
				 result.put("status", "success");
				 result.put("successMessage","Answer Stored SuccessFully");
				 return result;
