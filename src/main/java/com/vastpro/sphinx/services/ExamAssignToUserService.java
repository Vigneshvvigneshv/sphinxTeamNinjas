...
public class ExamAssignToUserService {
	...
	public static Map<String,Object> assignExam(DispatchContext context, Map<String,Object> input){
		...
		try {
			GenericValue partyIdAlreadyExits = EntityQuery.use(delegator).from("UserLogin").where("partyId",input.get("partyId")).queryFirst();
			if(partyIdAlreadyExits==null) {
				return ServiceUtil.returnError("User not found");
			}
			GenericValue examIdAlreadyExits=EntityQuery.use(delegator).from("ExamMaster").where("examId",input.get("examId")).queryFirst();
